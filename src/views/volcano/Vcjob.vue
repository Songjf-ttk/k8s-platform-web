<template>
    <div class="vcjob">
      <el-row>
        ...
        <el-col :span="24">
          <div>
            <el-card class="vcjob-head-card" shadow="never" :body-style="{padding:'10px'}">
              <el-row>
                <el-col :span="6">
                  <div>
                    <span>命名空间: </span>
                ...
                <el-col :span="2" :offset="16">
                  <div>
                    <el-button style="border-radius:2px;" icon="Refresh" plain @click="getvcjobs()">刷新</el-button>
                  </div>
                </el-col>
              </el-row>
            </el-card>
          ...
          <div>
            <el-card class="vcjob-head-card" shadow="never" :body-style="{padding:'10px'}">
              <el-row>
                ...
                <el-col :span="6">
                  <div>
                    <el-input class="vcjob-head-search" clearable placeholder="请输入" v-model="searchInput"></el-input>
                    <el-button style="border-radius:2px;" icon="Search" type="primary" plain @click="getvcjobs()">搜索</el-button>
                  </div>
                </el-col>
              </el-row>
            </el-card>
        ...
        <el-col :span="24">
          <div>
            <el-card class="vcjob-body-card" shadow="never" :body-style="{padding:'5px'}">
              <!-- 数据表格 -->
              ...
              <el-table
                      ...
                      <el-tab-pane label="容器" name="container">
                        <el-card shadow="never" style="border-radius:1px;" :body-style="{padding:'5px'}">
                          ...
                          <el-table
                            ...
                            <el-table-column align=center label="vcjob IP">
                              <span>{{ props.row.status.vcjobIP }}</span>
                            </el-table-column>
                      ...
                      <el-tab-pane label="日志" name="log">
                        <el-card shadow="never" style="border-radius:1px;" :body-style="{padding:'5px'}">
                          <el-row :gutter="10">
                            ...
                            <el-col :span="2">
                              <!-- 查看日志按钮 -->
                              <el-button style="border-radius:2px;" size="small" type="primary" @click="getvcjobLog(props.row.metadata.name)">查看</el-button>
                            </el-col>
                            <el-col :span="24" style="margin-top: 5px">
                              <!-- 显示日志内容 -->
                              <el-card shadow="never" class="vcjob-body-log-card" :body-style="{padding:'5px'}">
                                <span class="vcjob-body-log-span">{{ logContent }}</span>
                              </el-card>
                            </el-col>
                          </el-row>
                        </el-card>
                      ...
                      <el-tab-pane label="终端" name="shell">
                        <el-card shadow="never" style="border-radius:1px;" :body-style="{padding:'5px'}">
                          <el-row :gutter="10">
                            ...
                            <el-col :span="24" style="margin-top: 5px">
                              <el-card shadow="never" class="vcjob-body-shell-card" :body-style="{padding:'5px'}">
                                <!-- xterm虚拟终端 -->
                                <div id="xterm"></div>
                              </el-card>
                            </el-col>
                ...
                <el-table-column align=left label="vcjob名">
                  <template v-slot="scope">
                    <!-- 三元运算：expandMap[scope.row.metadata.name]为1则
                    触发关闭（expandedRows为空数组），为0则触发展开expandedRows有值 -->
                    <a class="vcjob-body-vcjobname" @click="expandMap[scope.row.metadata.name] ? expandChange(scope.row, []) : expandChange(scope.row, [scope.row])">{{ scope.row.metadata.name }}</a>
                  </template>
                </el-table-column>
                ...
                <el-table-column align=center label="操作" width="200">
                  <template v-slot="scope">
                    <el-button size="small" style="border-radius:2px;" icon="Edit" type="primary" plain @click="getvcjobDetail(scope)">YAML</el-button>
                    <el-button size="small" style="border-radius:2px;" icon="Delete" type="danger" @click="handleConfirm(scope, '删除', delvcjob)">删除</el-button>
                  </template>
                </el-table-column>
              </el-table>
              <el-pagination
                  class="vcjob-body-pagination"
                  background
      ...
      <el-dialog title="YAML信息" v-model="yamlDialog" width="45%" top="5%">
        ...
        <template #footer>
                  <span class="dialog-footer">
                      ...
                      <el-button type="primary" @click="updatevcjob()">更 新</el-button>
                  </span>
        </template>
      </el-dialog>
    </div>
  ...
  export default {
    data() {
      return {
        ...
        containerList: {},
        containerValue: '',
        getvcjobContainerData: {
          url: common.k8svcjobContainer,
          params: {
            vcjob_name: '',
            namespace: ''
          }
    ...
    methods: {
      ...
      getvcjobs() {
        ...
        this.getvcjobsData.params.filter_name = this.searchInput
        this.getvcjobsData.params.namespace = this.namespaceValue
        this.getvcjobsData.params.page = this.currentPage
        this.getvcjobsData.params.limit = this.pagesize
        httpClient.get(this.getvcjobsData.url, {params: this.getvcjobsData.params})
            .then(res => {
              this.vcjobList = res.data.items
              this.vcjobTotal = res.data.total
            })
            .catch(res => {
              this.$message.error({
                message: res.msg
      ...
      getvcjobDetail(e) {
        this.getvcjobDetailData.params.vcjob_name = e.row.metadata.name
        this.getvcjobDetailData.params.namespace = this.namespaceValue
        httpClient.get(this.getvcjobDetailData.url, {params: this.getvcjobDetailData.params})
            .then(res => {
              this.vcjobDetail = res.data
              this.contentYaml = this.transYaml(this.vcjobDetail)
      ...
      updatevcjob() {
        ...
        this.updatevcjobData.params.namespace = this.namespaceValue
        this.updatevcjobData.params.content = content
        httpClient.put(this.updatevcjobData.url, this.updatevcjobData.params)
            .then(res => {
      ...
      delvcjob(e) {
        this.delvcjobData.params.vcjob_name = e.row.metadata.name
        this.delvcjobData.params.namespace = this.namespaceValue
        httpClient.delete(this.delvcjobData.url, {data: this.delvcjobData.params})
            .then(res => {
              this.getvcjobs()
              this.$message.success({
                message: res.msg
      ...
      expandChange(row, expandedRows) {
        ...
        if (expandedRows.length > 0) {
          ...
          this. getvcjobContainer(row)
        } else {
          //关闭标记
          this.expandMap[row.metadata.name] = 0
        }
      },
      //匹配expandMap中vcjobName，不相等的全都置为0，意为除了vcjobName这行，其他全都标记关闭
      setExpandMap(vcjobName) {
        let key
        for ( key in this.expandMap ) {
          key !== vcjobName ? this.expandMap[key] = 0 : ''
        }
      },
      getvcjobContainer(row) {
        this.getvcjobContainerData.params.vcjob_name = row.metadata.name
        this.getvcjobContainerData.params.namespace = this.namespaceValue
        httpClient.get(this.getvcjobContainerData.url, {params: this.getvcjobContainerData.params})
            .then(res => {
      ...
      getvcjobLog(vcjobName) {
        this.getvcjobLogData.params.vcjob_name = vcjobName
        this.getvcjobLogData.params.container_name = this.containerValue
        this.getvcjobLogData.params.namespace = this.namespaceValue
        httpClient.get(this.getvcjobLogData.url, {params: this.getvcjobLogData.params})
            .then(res => {
      ...
      initSocket(row) {
        //定义websocket连接地址
        let terminalWsUrl = common.k8sTerminalWs + "?vcjob_name=" + row.metadata.name + "&container_name=" + this.containerValue + "&namespace=" + this.namespaceValue
        //实例化
      ...
      activeName: {
        handler() {
          if ( this.activeName == 'log' ) {
            this.expandKeys.length == 1 ? this.getvcjobLog(this.expandKeys[0]) : ''
          }
        }
      }
    },
  ...
  <style scoped>
  .vcjob-head-card,.vcjob-body-card {
    border-radius: 1px;
    margin-bottom: 5px;
  }
  .vcjob-head-search {
  ...
  .vcjob-body-vcjobname {
    color: #4795EE;
  }
  .vcjob-body-vcjobname:hover {
  ...
  .vcjob-body-log-card, .vcjob-body-shell-card {
    border-radius:1px;
    height:600px;
    overflow:auto;
